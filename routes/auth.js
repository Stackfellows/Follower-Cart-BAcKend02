...
module.exports = (transporter) => {
  ...
  router.post("/login", async (req, res) => {
    try {
      ...
      if (!user.password) {
        return res.status(400).json({
          msg: "This account was created using Google login. Please use 'Continue with Google'.",
        });
      }
  ...
  // get//blogposts for admin and user
  router.get("/blogPosts", async (req, res) => {
    try {
      const posts = await BlogPost.find().sort({ createdAt: -1 }); // optional sorting
      res.json(posts);
    } catch (err) {
      console.error("Error fetching blog posts:", err);
      res
        .status(500)
        .json({ msg: "Server error fetching blog posts.", error: err.message });
    }
  });

  // POST create blog post (with optional image)
  router.post("/blogPosts", upload.single("image"), async (req, res) => {
    try {
      ...
      const newPost = new BlogPost({
        title,
        content,
        author: req.user?.name || "Admin", // Optional: You can attach user via middleware
        imageUrl: req.file
  ...
  router.put("/blogPosts/:id", upload.single("image"), async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ msg: "Server error updating blog post.", error: err.message });
    }
  });

  // ... aapke existing routes ke baad yeh code add karein

  // ============================= // Refund Request Routes // =============================

  // NEW: Route to handle refund requests submission

  // Route to handle refund requests
  router.post("/refundRequests", async (req, res) => {
    try {
      // Deconstruct the body
      const { orderId, clientName, clientEmail, amount, reason } = req.body;

      // Validate required fields and ensure orderId is not an empty string
      if (
        !orderId ||
        orderId.trim() === "" ||
        !clientName ||
        ...
        return res
          .status(400)
          .json({ msg: "Please fill all fields, including a valid Order ID." });
      }
      ...
      res.status(201).json({
        msg: "Refund request submitted successfully! We will get back to you shortly.",
        request: newRefundRequest,
      });
    } catch (err) {
      console.error("Error submitting refund request:", err);

      // Check for the specific duplicate key error and provide a user-friendly message
      if (err.code === 11000) {
        return res.status(409).json({
          msg: "A refund request for this order ID has already been submitted.",
          error: err.message,
        });
      }

      res.status(500).json({
        msg: "Server error occurred while submitting refund request.",
        error: err.message,
      });
    }
  });
  ...
  // ... existing routes
  router.get("/refundRequests", async (req, res) => {
    try {
      // Use .find() without populating order to avoid errors if an order is deleted
